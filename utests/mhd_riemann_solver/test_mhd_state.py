...
class TestWaveSpeeds_Ordering(unittest.TestCase):
    ...
    def test_slow_le_alfven_le_fast_for_ryu_jones_2a_right_state(
        ...
        c_fast, c_slow = mhd_state.compute_fast_slow_speeds(state=_RIGHT_STATE, magnetic_field_normal=_MAGNETIC_FIELD_NORMAL, gamma=_GAMMA)
        c_alfven = mhd_state.compute_alfven_speed(state=_RIGHT_STATE, magnetic_field_normal=_MAGNETIC_FIELD_NORMAL)
        self.assertLessEqual(c_slow, c_alfven + 1e-12)
        self.assertLessEqual(c_alfven, c_fast + 1e-12)
...
class TestWaveSpeeds_HydroLimit(unittest.TestCase):
    ...
    def test_zero_field_recovers_sound_speed(
        ...
        c_fast, c_slow = mhd_state.compute_fast_slow_speeds(state=state, magnetic_field_normal=0.0, gamma=_GAMMA)
        c_sound = mhd_state.compute_sound_speed(state=state, gamma=_GAMMA)
...
class TestConservedPrimitive_RoundTrip(unittest.TestCase):
    ...
    def test_as_conserved_and_back_recovers_state(
        ...
        conserved = mhd_state.as_conserved(state=_LEFT_STATE, magnetic_field_normal=_MAGNETIC_FIELD_NORMAL, gamma=_GAMMA)
        recovered = mhd_state.as_primitive(conserved=conserved, magnetic_field_normal=_MAGNETIC_FIELD_NORMAL, gamma=_GAMMA)
        ...
        self.assertAlmostEqual(recovered.magnetic_field_transverse_1, _LEFT_STATE.magnetic_field_transverse_1, places=12)
        self.assertAlmostEqual(recovered.magnetic_field_transverse_2, _LEFT_STATE.magnetic_field_transverse_2, places=12)
